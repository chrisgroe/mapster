...
class PolarTransformationTest : XCTestCase {
    func testForwardBack() {
        ...
        for test in testVector {
            ...
            let (radius, rho) = PolarTransformation.cartesianToPolar(x,y)
            let (xAct, yAct) = PolarTransformation.polarToCartesian(radius, rho)
    ...
    func testBackForward() {
        ...
        for test in testVector {
            ...
            let (x, y) = PolarTransformation.polarToCartesian(radius,phi)
            let (radiusAct, phiAct) = PolarTransformation.cartesianToPolar(x,y)
    ...
    func testForwardBackInt() {
        ...
        for test in testVector {
            ...
            let (radius, rho) = PolarTransformation.discreteCartesianToPolar(x,y)
            let (xAct, yAct) = PolarTransformation.polarToDiscreteCartesian(radius, rho)
            actualVector.append((xAct, yAct))
